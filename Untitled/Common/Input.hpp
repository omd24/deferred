...
struct KeyState
{
  ...
  KeyState() : Pressed(false), RisingEdge(false), FallingEdge(false) {}
};
...
public:
  KeyState GetKeyState(Keys key) const { return keyStates[key]; }
  bool IsKeyDown(Keys key) const { return keyStates[key].Pressed; }
  bool RisingEdge(Keys key) const { return keyStates[key].RisingEdge; }
...
private:
  ...
  static inline BYTE currState[256];
