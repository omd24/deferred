...
void ShadeSample(in uint2 pixelPos)
{
    ...
    float3 positionDX = PosWSFromDepth(zwNeighbors.x, screenUV + (int2(1, 0) * invRTSize)) - positionWS;
    float3 positionDY = PosWSFromDepth(zwNeighbors.y, screenUV + (int2(0, 1) * invRTSize)) - positionWS;
